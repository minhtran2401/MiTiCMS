        ...
        <div id="sidebar-menu" class="sidebar-menu">
            <ul>
                ...
                <li class="submenu {{ request()->is('ten-mien/*','ten-mien') ? 'active' : '' }}">
                    <a class="{{ request()->is('ten-mien/*','ten-mien') ? 'active' : '' }}" href="#"><i class="fa fa-user"></i> <span> Tên Miền </span> <span class="menu-arrow"></span></a>
                    <ul style="display: none;">
                ...
                <li class="submenu {{ request()->is('server/*') ? 'active' : '' }}">
                    <a class="{{ request()->is('server/*') ? 'active' : '' }}" href="#"><i class="fa fa-user"></i> <span> Máy Chủ Vật Lí </span> <span class="menu-arrow"></span></a>
                    <ul style="display: none;">
                        @php 
                            $servers = DB::table('service_types')->where('service_group_id','1')->where('display','1')->orderby('service_type_id','desc')->get();
                        @endphp
                        @foreach ($servers as $server)
                              <li><a href="{{route('server.server-type',$server->slug)}}">{{$server->service_type_name}}</a></li>
                        @endforeach
                ...
                <li class="submenu {{ request()->is('vps/*') ? 'active' : '' }}">
                    <a class="{{ request()->is('vps/*') ? 'active' : '' }}" href="#"><i class="fa fa-user"></i> <span> VPS </span> <span class="menu-arrow"></span></a>
                    <ul style="display: none;">
                        @php 
                ...
                <li class="submenu {{ request()->is('hosting/*') ? 'active' : '' }}">
                    <a class="{{ request()->is('hosting/*') ? 'active' : '' }}" href="{{route('hosting.index')}}"><i class="fa fa-user"></i> <span>Hosting </span> <span class="menu-arrow"></span></a>
                    <ul style="display: none;">
                        @php 
                ...
                <li class="submenu {{ request()->is('tai-khoan/*') ? 'active' : '' }}">
                    <a class="{{ request()->is('tai-khoan/*') ? 'active' : '' }}" href="{{route('hosting.index')}}"><i class="fa fa-user"></i> <span>Tài khoản các loại </span> <span class="menu-arrow"></span></a>
                    <ul style="display: none;">
                        @php 
                        $accounts = DB::table('service_types')->where('service_group_id','4')->where('display','1')->orderby('service_type_id','desc')->get();
                    @endphp
                    @foreach ($accounts as $account)
                          <li><a href="{{route('account.account-type',$account->slug)}}">{{$account->service_type_name}}</a></li>
                    @endforeach
                ...
                <li class="{{ request()->is('huong-dan-thanh-toan') ? 'active' : '' }}">
                    <a href="{{route('how_to_pay')}}"><i class="fa fa-user-md"></i> <span>Thanh Toán</span></a>
                </li>
