@extends('FE.layout')
@section('pagetitle', " $account_type->service_type_name ")
@section('content')
            ...
            <div class="col-lg-12">
                <div class="card" style="overflow: hidden">
                    <div class="shin-title-page">
                        <h3>MiTiVPS - VPS - {{$account_type->service_type_name}}</h3>
                    </div>
                    <div class="card-body">
                                    ...
                                    <div class="row justify-content-center mb-5">
                                        <div class="col-12 text-center heading-section ftco-animate">
                                            ...
                                            <div class="row justify-content-center mb-3">
                                                <div class="">
                                                    @php 
                            $accounts = DB::table('service_types')->where('service_group_id','4')->where('display','1')->get();
                        @endphp
                        @foreach ($accounts as $account)
                         <div class="shin-service-menu-type"><a style="color: white" href="{{route('account.account-type',$account->slug)}}">{{$account->service_type_name}}</a></div>
                                                            ...
                                                            <div style="box-shadow: 0 0 3px grey;" class="card block-7">
                                                                <div class="card-body">
                                                                    <div class="text-center">
                                                                        <form class="buy_service" action="{{route('addcart.account',$detail->sku)}}" method="post">
                                                                          @csrf
                                                                        <h2 class="heading">{{$detail->name}}</h2>
                                                                        <div class="shin-service-type mb-2">{{$account_type->service_type_name}}</div>
                                                                        <div class="shin-service-title-info mb-2">Thông tin cấu hình</div>
                                                                        <div class="shin-service-info mb-2">
                                                                           {!!$detail->account_profile!!}
                                                                        </div>
                                                          ...
                                                          <div class="modal-dialog modal-dialog-centered" role="document">
                                                            <div class="modal-content">
                                                              <div class="modal-header">
                                                                <h5 class="modal-title" id="exampleModalLongTitle">Giá gói {{$account_type->service_type_name}}</h5>
...
var reurl = "{{route('purchase_account')}}";
$.ajax({
       type: "POST",
       url: url,
