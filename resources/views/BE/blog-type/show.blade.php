...
@endsection
@section('br-namepage1','Loại Blog')
@section('br-namepage2','Loại Blog')
@section('br-namepage3','Loại Blog')
                ...
                <div class="col-12">
                    <div class="alert alert-primary" role="alert">
                        <div class="alert-body">
                            <strong>Ghi chú:</strong> Loại Blog của website.
          ...
          <section id="basic-data" >
                    ...
                    <div class="card p-2">
                        ...
                        <table class="datatables-basic table" id="table-1">
                            ...
                            <tbody>
                                @foreach ($ds as $row)
                                <tr>
                                  <th scope="row">{{$row->blog_type_id}}</th>
                                  <td>{{$row->blog_type_name}}</td>

                                  <td data-id="{{ $row->blog_type_id }}">
                                    <div class="custom-control custom-switch custom-control-inline">
                                        <input type="checkbox" id="gr-{{$row->blog_type_id}}"  class="custom-control-input change-status"  {{ $row->display==1?'checked':'' }}>
                                        <label for="gr-{{$row->blog_type_id}}" class="custom-control-label content-status" >{{ $row->display==1?'Hiện':'Ẩn'}}</label>
                                    </div>
                                  </td>

                                  <td >
                                    <div class="modal fade text-left" id="editgr-{{$row->blog_type_id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title" id="myModalLabel33">Sửa Loại Blog</h4>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <form class="form-change-group" action="{{route('blog-type.update', $row->blog_type_id)}}" method="POST">
                                                    @csrf
                                                    @method('patch')
                                    ...
                                    <button  class="btn btn-icon btn-primary" data-toggle="modal" data-target="#editgr-{{$row->blog_type_id}}" > <i class="fa fa-eye" aria-hidden="true"></i> Sửa</button>
                                  
                                  <form class="form-check-inline" action="{{route('blog-type.destroy', $row->blog_type_id)}}" method="POST">
                                        @csrf
                                        @method('delete')
            ...
            <div class="modal modal-slide-in fade" id="modals-slide-in">
                <div class="modal-dialog sidebar-sm">
                    <form id="create-groups-service" method="POST" action="{{route('store.blt.ajax')}}" class="add-new-record modal-content pt-0">
                        @csrf
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">×</button>
                        <div class="modal-header mb-1">
                            <h5 class="modal-title" id="exampleModalLabel">Thêm Loại Blog Mới</h5>
                        </div>
                        <div class="modal-body flex-grow-1">
                            <div class="form-group">
                                <label class="form-label" for="basic-icon-default-fullname">Tên Nhóm</label>
                                <input type="text" name="blog_type_name" class="form-control dt-full-name" id="basic-icon-default-fullname" placeholder="Tên Nhóm" required  />
                            </div>
    ...
    $('.change-status').change(function(e){
        ...
        var status=$(this).prop('checked')?1:0;
          ...
          if(id){
              $.ajax({
                  //tên route có url là ....
                  url:"{{ route('changeStatus.blog-type') }}",
