...
@section('pagetitle','Dịch Vụ Server')
@section('csspage')
...
@endsection
@section('content')
@section('br-namepage1','Server')
@section('br-namepage2','Dịch vụ')
@section('br-namepage3','Dịch vụ Server')
                ...
                <div class="col-12">
                    <div class="alert alert-primary" role="alert">
                        <div class="alert-body">
                            <strong>Ghi chú:</strong> Dịch vụ Server của website.
                ...
                <div class="col-12">
                    <div class="card p-2">
                        ...
                        <div class="dt-action-buttons text-right"><div class="dt-buttons d-inline-flex">
                              ...
                              <a  class="btn btn-outline-primary" href="{{route('server.create')}}"> + Thêm Server</a>
                        ...
                        <table class="datatables-basic table" id="table-1">
                            ...
                            <tbody>
                                @foreach ($ds as $row)
                                <tr>
                                  <th scope="row">{{$row->server_id}}</th>
                                  <td>
                                    <img src="{{asset('server')}}/{{$row->server_image}}" class="mr-75" height="20" width="20" alt="Angular">
                                    <span class="font-weight-bold">{{$row->server_name}}</span>
                                </td>
                                <td>
                                  @php
                                ...
                                <td data-id="{{ $row->server_id }}">
                                  <div class="custom-control custom-switch custom-control-inline">
                                      <input type="checkbox" id="server-{{$row->server_id}}"  class="custom-control-input change-status"  {{ $row->display==1?'checked':'' }}>
                                      <label for="server-{{$row->server_id}}" class="custom-control-label content-status" >{{ $row->display==1?'Hiện':'Ẩn'}}</label>
                                  </div>
                                </td>
                                    ...
                                    <td>
                                      <form class="form-check-inline"  method="GET">
                                        {{ method_field('UPDATE')}}
                                        @csrf
                                        <a href="{{route('server.edit', $row->server_id)}}"  title="Sửa" class="btn btn-icon btn-primary"><i class="fa fa-eye" aria-hidden="true"></i> Sửa</a>
                                      </form>
                                  <form class="form-check-inline" action="{{route('server.destroy', $row->server_id)}}" method="POST">
                                        @csrf
                                        @method('delete')
    ...
    $('.change-status').change(function(e){
        ...
        var status=$(this).prop('checked')?1:0;
          ...
          if(id){
              $.ajax({
                  //tên route có url là ....
                  url:"{{ route('changeStatus.server-service') }}",
