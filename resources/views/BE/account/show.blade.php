...
@section('pagetitle','Dịch vụ tài khoản')
@section('csspage')
...
@section('br-namepage3','Dịch vụ tài khoản')
                ...
                <div class="col-12">
                    <div class="alert alert-primary" role="alert">
                        <div class="alert-body">
                            <strong>Ghi chú:</strong> Dịch vụ tài khoản của website.
          ...
          <section id="basic-data" >
                        ...
                        <table class="datatables-basic table" id="table-1">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Tài khoản</th>
                                    <th class="text-center">Combo giá</th>
                                    <th>Trạng thái</th>
                                    <th>Quản Lí</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <td>{{$row->account_id}}</td>
                                  ...
                                  <td data-id="{{ $row->account_id }}">
                                    <div class="custom-control custom-switch custom-control-inline">
                                        <input type="checkbox" id="ac-{{$row->account_id}}"  class="custom-control-input change-status"  {{ $row->display==1?'checked':'' }}>
                                        <label for="ac-{{$row->account_id}}" class="custom-control-label content-status" >{{ $row->display==1?'Hiện':'Ẩn'}}</label>
                                    </div>
                                  </td>
                                  ...
                                  <td >
                                    ...
                                    <form class="form-check-inline"  method="GET">
                                      {{ method_field('UPDATE')}}
                                      @csrf
                                      <a href="{{route('account.edit', $row->account_id)}}"  title="Sửa" class="btn btn-icon btn-primary"><i class="fa fa-eye" aria-hidden="true"></i> Sửa</a>
                                    </form>
                                  
                                  <form class="form-check-inline" action="{{route('account.destroy', $row->account_id)}}" method="POST">
                                        @csrf
                                        @method('delete')
            ...
            <div class="modal modal-slide-in fade" id="modals-slide-in">
                <div class="modal-dialog sidebar-sm">
                    <form id="create-domain" method="POST" action="{{route('store.ac.ajax')}}" class="add-new-record modal-content pt-0">
                        @csrf
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">×</button>
                        <div class="modal-header mb-1">
                            <h5 class="modal-title" id="exampleModalLabel">Thêm Nhanh Tài Khoản</h5>
                        </div>
    ...
    $('.change-status').change(function(e){
        ...
        var status=$(this).prop('checked')?1:0;
          ...
          if(id){
              $.ajax({
                  //tên route có url là ....
                  url:"{{ route('changeStatus.account-service') }}",
